...
coefficient <- c(seq(from = 0, to = 1.6, by = 0.2), NA)
param <- expand.grid(row = coefficient, col = coefficient) %>%
	filter(xor(is.na(row), is.na(col)) | (row == col & !is.na(row)))
...
for (i in seq_len(nrow(param))) {
	...
	for (j in 1:nrow(row_sum)) {
		...
		result[[length(result) + 1]] <- c(res$final, instance = j, param[i, c("row", "col")])
	}
}

df <- bind_rows(result)

write.csv(df, gzfile("__data__/__results__3_data_munu_constraint__.csv.gz"))
